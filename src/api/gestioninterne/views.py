...
def execute_change(cyclos, change):
    try:
        ...
        try:
            ...
            bank_transfer_reference = datetime.now().strftime("%d/%m/%Y %H:%M:%S") + '-' + change['member_login']
        ...
        change['status'] = 1
        change['message'] = _("Change effectué")
    except Exception as e:
        change['status'] = 0
